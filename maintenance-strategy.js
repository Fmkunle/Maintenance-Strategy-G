...
const initialStrategyTableColumnKeys = [
  "physicalAssetName",
  ...
  "failureModeEffectRedundancyFactor",
  "failureModeIsDormant",
  "failureModeDemandFrequency",
  ...
  "scheduledTaskDetectionProbability",
  "scheduledTaskDuration",
  "scheduledTaskLaborLabor",
  "failureModeAlarmIsEnabled",
  "failureModeAlarmDescription",
  "failureModeAlarmPfInterval",
  "failureModeAlarmDetectionProbability",
  "failureModeCostBenefitRatio",
  "failureModeTotalCost",
  "failureModeEffectCost",
  "failureModeCorrectiveDownTime",
  "failureModeCorrectiveEventCount",
  "failureModeCorrectiveCost",
  "failureModePlannedCost",
  "failureModeSecondaryActionCost",
  "failureModeInspectionCost",
  "failureModeFailureRate",
  "failureModeAvailability",
];

const initialVisibleStrategyTableColumnKeys = [
  "physicalAssetName",
  "physicalAssetDescription",
  "componentName",
  "failureModeName",
...
const defaultState = () => ({
  ...
  collapsedNodeIds: [],
  hierarchyFilter: "",
  strategyTable: createInitialStrategyTableState(),
...
const isChildDraftReady = () => {
  ...
  if (childDraftState.childType === "effect") {
    ...
    const editingNodeCode = childDraftState.editNodeId
      ? String(findNodeInfo(state.hierarchy, childDraftState.editNodeId)?.node?.code || "").trim()
      : "";
    return parentInfo ? !getUsedEffectCodesForParent(parentInfo.node).has(effectOption.code) || editingNodeCode === effectOption.code : true;
  }
...
const createExistingHierarchyNodeDraft = (nodeInfo) => {
  if (!nodeInfo) {
    return defaultChildDraftState();
  }
  ...
  return {
    ...
    editNodeId: node.id,
    childType: node.type,
    codeSegment: String(node.code || "").trim(),
    description: getNodeDescription(node),
  };
};
const isRightPaneInspectModeForNode = (nodeInfo) => Boolean(nodeInfo && rightPaneInspectState.nodeId === nodeInfo.node.id);
const openRightPaneInspectMode = (nodeId) => {
  closeChildCreator();
  closeEquipmentInfo();
  closeCauseConfig();
  closePmConfig();
  closeInsConfig();
  rightPaneInspectState = {
    nodeId: String(nodeId || ""),
  };
};
const closeRightPaneInspectMode = () => {
  rightPaneInspectState = {
    nodeId: "",
  };
};
...
const openExistingHierarchyNodeEditor = (nodeInfo) => {
  if (!nodeInfo || ["equipment", "cause", "cm", "pm", "ins"].includes(nodeInfo.node.type)) {
    closeChildCreator();
    return;
  }
  ...
  closePmConfig();
  closeInsConfig();
  childDraftState = createExistingHierarchyNodeDraft(nodeInfo);
};
...
const saveEquipmentInfo = (nodeId, draft) => {
  ...
  if (rightPaneInspectState.nodeId === nodeId) {
    closeEquipmentInfo();
  } else {
    equipmentInfoState = {
      mode: "view",
...
const getNodeInspectValueMarkup = (label, value, isWide = false) => `
  <div class="node-inspect-panel__item ${isWide ? "node-inspect-panel__item--wide" : ""}">
    ...
    <strong class="node-inspect-panel__value ${hasValue(value) ? "" : "is-empty"}">${escapeHtml(getDisplayValue(value, "Not set"))}</strong>
  </div>
`;
const getNodeInspectPathLabel = (nodeInfo) =>
  (nodeInfo?.path || [])
    .map((entry, index) => getLeftPanelRowName(entry, nodeInfo.path.slice(0, index + 1)))
    .join(" / ");
...
const renderNodeInspectPanel = (nodeInfo) => {
  ...
  const definitionMarkup = `
    <section class="asset-child-creator__section">
      ...
      <div class="node-inspect-panel__grid">
        ${getNodeInspectValueMarkup("Type", getNodeLabel(node))}
        ${getNodeInspectValueMarkup("Code", getNodeCodeValue(node))}
        ${getNodeInspectValueMarkup("Name", getLeftPanelRowName(node, path))}
        ${getNodeInspectValueMarkup("Description", getLeftPanelRowDescription(node, path), true)}
        ${getNodeInspectValueMarkup("Path", pathLabel, true)}
      </div>
    </section>
  `;

  let detailSectionsMarkup = `
    <section class="asset-child-creator__section">
      <header class="asset-child-creator__section-head">
        <strong class="asset-child-creator__section-title">Context</strong>
      </header>
      <div class="node-inspect-panel__grid node-inspect-panel__grid--triple">
        ${getNodeInspectValueMarkup("Parent", parent ? getLeftPanelRowName(parent, path.slice(0, -1)) : "Root node")}
        ${getNodeInspectValueMarkup("Children", String((node.children || []).length))}
        ${getNodeInspectValueMarkup("Related strategies", String(strategyRowCount))}
      </div>
    </section>
  `;
  ...
  if (node.type === "equipment") {
    ...
    detailSectionsMarkup += `
      <section class="asset-child-creator__section">
        ...
        <div class="node-inspect-panel__grid">
          ${getNodeInspectValueMarkup("Equipment Function", context.equipmentFunction)}
          ${getNodeInspectValueMarkup("Type of Equipment", context.equipmentType)}
          ${getNodeInspectValueMarkup("Operating Context", context.operatingContext, true)}
        </div>
      </section>
      <section class="asset-child-creator__section">
        <header class="asset-child-creator__section-head">
        ...
        <div class="node-inspect-panel__grid node-inspect-panel__grid--triple">
          ${getNodeInspectValueMarkup("Effect", context.effectPerHourDown)}
          ${getNodeInspectValueMarkup("Demand Frequency", context.demandFrequency)}
          ...
          ${getNodeInspectValueMarkup("MAE", context.maeCategory)}
          ${getNodeInspectValueMarkup("Criticality", context.criticality)}
        </div>
      </section>
    `;
    ...
    detailSectionsMarkup += `
      <section class="asset-child-creator__section">
        ...
        <div class="node-inspect-panel__grid node-inspect-panel__grid--triple">
          ${getNodeInspectValueMarkup("Component", config.componentName)}
          ${getNodeInspectValueMarkup("Distribution", config.distribution)}
          ${getNodeInspectValueMarkup("Demand Frequency", config.demandFrequency)}
          ${getNodeInspectValueMarkup("MTTF", config.mttf)}
          ${getNodeInspectValueMarkup("Dormant", getNodeInspectBooleanLabel(config.isDormant))}
          ${getNodeInspectValueMarkup("Alarm", config.alarmDescription)}
        </div>
      </section>
    `;
    ...
    detailSectionsMarkup += `
      <section class="asset-child-creator__section">
        <header class="asset-child-creator__section-head">
          <strong class="asset-child-creator__section-title">Effect Details</strong>
        </header>
        <div class="node-inspect-panel__grid node-inspect-panel__grid--triple">
          ${getNodeInspectValueMarkup("Effect", effectJson?.["Failure Mode Effect Effect"] || getNodeDescription(node), true)}
          ${getNodeInspectValueMarkup("Redundancy Factor", effectJson?.["Failure Mode Effect Redundancy Factor"] || "")}
          ${getNodeInspectValueMarkup("Effect Cost", effectJson?.["Failure Mode Effect Cost"] || "")}
        </div>
      </section>
    `;
    ...
    detailSectionsMarkup += `
      <section class="asset-child-creator__section">
        <header class="asset-child-creator__section-head">
          <strong class="asset-child-creator__section-title">Task Details</strong>
        </header>
        <div class="node-inspect-panel__grid node-inspect-panel__grid--triple">
          ${getNodeInspectValueMarkup("Task Name", taskJson?.["Task Name"] || getNodeCodeValue(node))}
          ${getNodeInspectValueMarkup("Task Type", taskJson?.["Scheduled Task Type"] || taskConfig.type || taskConfig.scheduledTaskType || "")}
          ${getNodeInspectValueMarkup(
            "Enabled",
...
const renderExistingHierarchyNodeEditor = (nodeInfo) => {
  if (!childCreatorPanel || !childDraftState.isOpen || childDraftState.editNodeId !== nodeInfo?.node?.id) {
    return;
  }
  ...
  const effectOptionsMarkup = effectCatalog
    .map((option) => {
      const isSelected = childDraftState.codeSegment === option.code;
      const isUsed = siblingEffectCodes.has(option.code) && !isSelected;
      return `<option value="${escapeHtml(option.code)}" ${isSelected ? "selected" : ""} ${isUsed ? "disabled" : ""}>${escapeHtml(
        getEffectCatalogLabel(option)
      )}${isUsed ? " (Already added)" : ""}</option>`;
    })
    .join("");

  const definitionSectionMarkup = isEffectNode
    ? `
        <section class="asset-child-creator__section">
          <header class="asset-child-creator__section-head">
            <strong class="asset-child-creator__section-title">Definition</strong>
      ...
      ? `
          <section class="asset-child-creator__section">
            ...
            <div class="asset-child-creator__preview-grid">
              <div class="asset-child-creator__preview-item">
                <span>Name</span>
                <strong>${escapeHtml(getNodeFullCode(node, path))}</strong>
              </div>
            </div>
            <label class="field field--full">
              <span>${getRequiredFieldLabel(getChildDescriptionLabel(node.type))}</span>
              <input
                id="childCreatorDescriptionInput"
                type="text"
                value="${escapeHtml(childDraftState.description)}"
                placeholder="Enter ${escapeHtml(getNodeLabel(node).toLowerCase())} description"
                required
              >
            </label>
          </section>
          ...
          <section class="asset-child-creator__section">
            ...
            <div class="asset-child-creator__row-grid">
              <label class="field">
                ...
                <div class="hierarchy-code-field ${inheritedPrefix ? "has-prefix" : ""}">
                  <span class="hierarchy-code-field__prefix">${escapeHtml(inheritedPrefix)}</span>
                  <input id="childCreatorNameInput" type="text" value="${escapeHtml(childDraftState.codeSegment)}" placeholder="Enter code segment" required>
                </div>
              </label>
              <label class="field">
                <span>${getRequiredFieldLabel("Description")}</span>
  ...
  childCreatorPanel.innerHTML = `
    <section class="asset-child-creator__form">
      ...
      <div class="asset-child-creator__actions">
        <button id="cancelChildCreatorButton" class="secondary-button" type="button">Cancel</button>
        <button id="createChildButton" class="primary-button" type="button" ${isChildDraftReady() ? "" : "disabled"}>Save</button>
      </div>
    </section>
  `;
};
const renderInspectNodeActions = (nodeInfo) => {
  ...
  selectedNodeActions.innerHTML = `
    ...
    <button class="primary-button" type="button" data-open-node-inspect-editor="${escapeHtml(nodeInfo.node.id)}">Edit</button>
  `;
};
...
const renderSelectedNodePanel = () => {
  ...
  const isEditingSelectedNode = Boolean(childDraftState.editNodeId) && childDraftState.editNodeId === node.id;
  const isInspectMode = isRightPaneInspectModeForNode(nodeInfo);
  if (childDraftState.editNodeId && childDraftState.editNodeId !== node.id) {
    closeChildCreator();
  }
  if (childDraftState.parentId && childDraftState.parentId !== node.id && !isEditingSelectedNode) {
    closeChildCreator();
  }
  ...
  const isGenericNodeEditorMode =
    !["equipment", "cause", "cm", "pm", "ins"].includes(node.type) && childDraftState.editNodeId === node.id && childDraftState.isOpen;
  ...
  if (equipmentInfoMode === "view" && !isInspectMode) {
  ...
  if (isGenericNodeEditorMode) {
    selectedNodeTypeLabel.textContent = `Edit ${getNodeLabel(node)}`;
    backgroundDetailHeading.textContent = getNodeDisplayName(node);
    backgroundDetailSummary.textContent = "";
    renderExistingHierarchyNodeEditor(nodeInfo);
    renderSelectedNodeActions(nodeInfo, { isAddMode: false, equipmentInfoMode: "closed", isEditorMode: true });
    if (strategyList) {
      strategyList.hidden = true;
      strategyList.innerHTML = "";
  ...
  if (isInspectMode) {
    ...
    backgroundDetailHeading.textContent = getNodeDisplayName(node);
    backgroundDetailSummary.textContent = getLeftPanelRowDescription(node, path);
    renderNodeInspectPanel(nodeInfo);
    renderInspectNodeActions(nodeInfo);
    if (strategyList) {
      strategyList.hidden = true;
      strategyList.innerHTML = "";
    }
...
const saveExistingHierarchyNode = (nodeId, draft) => {
  const info = findNodeInfo(state.hierarchy, nodeId);
  if (!info || ["equipment", "cause", "cm", "pm", "ins"].includes(info.node.type)) {
    return;
  }
  ...
  if (info.node.type === "effect") {
    const effectOption = getEffectCatalogOption(draft?.codeSegment || "");
    if (!effectOption) {
      return;
    }
    const siblingCodes = info.parent ? getUsedEffectCodesForParent(info.parent) : new Set();
    if (effectOption.code !== info.node.code && siblingCodes.has(effectOption.code)) {
      return;
    }
    info.node.code = effectOption.code;
    info.node.name = effectOption.code;
    info.node.description = effectOption.description;
  } else if (autoGeneratedChildTypes.has(info.node.type)) {
    info.node.description = String(draft?.description || "").trim();
  } else {
    const siblings = info.parent ? info.parent.children.filter((child) => child.id !== nodeId) : state.hierarchy.filter((node) => node.id !== nodeId);
    const nextCodeSegment = sanitizeCodeSegment(draft?.codeSegment || "", info.node.type, siblings);
    if (!nextCodeSegment) {
      return;
    }
    const parentFullCode = getParentFullCodeFromPath(info.path);
    info.node.code = nextCodeSegment;
    info.node.name = shortCodeHierarchyTypes.has(info.node.type)
      ? nextCodeSegment
      : joinInheritedCode(parentFullCode, nextCodeSegment, info.node.type);
    info.node.description = String(draft?.description || "").trim();
    (info.node.children || []).forEach((child) => {
      updateInheritedCodesForSubtree(child, info.node.name);
    });
...
const deleteHierarchyNode = (nodeId) => {
  ...
  if (childDraftState.editNodeId && deletedNodeIds.has(childDraftState.editNodeId)) {
    closeChildCreator();
  }
  ...
  if (pmConfigState.nodeId && deletedNodeIds.has(pmConfigState.nodeId)) {
    closePmConfig();
  }
  if (insConfigState.nodeId && deletedNodeIds.has(insConfigState.nodeId)) {
    closeInsConfig();
  }
  if (rightPaneInspectState.nodeId && deletedNodeIds.has(rightPaneInspectState.nodeId)) {
    closeRightPaneInspectMode();
  }
...
assetHierarchyTree?.addEventListener("dblclick", (event) => {
  const target = event.target;
  if (!(target instanceof Element)) {
    return;
  }

  if (target.closest("[data-toggle-collapse], [data-open-child-creator], .asset-register-row__checkbox")) {
    return;
  }

  const selectTarget = target.closest("[data-select-node]");
  if (!selectTarget) {
    return;
  }

  const nodeId = selectTarget.dataset.selectNode;
  if (!nodeId) {
    return;
  }
...
childCreatorPanel?.addEventListener("click", (event) => {
  ...
  if (cancelEquipmentInfoEditButton) {
    const nodeInfo = getSelectedNodeInfo();
    if (nodeInfo && rightPaneInspectState.nodeId === nodeInfo.node.id) {
      closeEquipmentInfo();
    } else {
  ...
  if (childDraftState.editNodeId) {
    if (["cm", "pm", "ins"].includes(childDraftState.childType)) {
      saveExistingTaskNode(childDraftState.editNodeId, childDraftState);
    } else {
      saveExistingHierarchyNode(childDraftState.editNodeId, childDraftState);
    }
    return;
  }
...
selectedNodeActions?.addEventListener("click", (event) => {
  const closeInspectButton = event.target.closest("[data-close-node-inspect]");
  ...
  const inspectEditButton = event.target.closest("[data-open-node-inspect-editor]");
  if (inspectEditButton) {
    event.stopPropagation();
    const nodeInfo = findNodeInfo(state.hierarchy, inspectEditButton.dataset.openNodeInspectEditor);
    if (nodeInfo) {
      state.selectedNodeId = nodeInfo.node.id;
      if (nodeInfo.node.type === "equipment") {
        openEquipmentInfoMode(nodeInfo, "edit");
      } else if (nodeInfo.node.type === "cause") {
        openCauseConfig(nodeInfo);
      } else if (["cm", "pm", "ins"].includes(nodeInfo.node.type)) {
