...
const createChildNode = (parentId, childType, draft) => {
  ...
  const fullCode = childType === "effect" ? codeSegment : joinInheritedCode(parentFullCode, codeSegment, childType);
  const description = childType === "effect" ? effectOption.description : String(draft?.description || "").trim();
  ...
  const nodeName = childType === "effect" ? codeSegment : fullCode;
  const nextNode = createNode(childType, codeSegment, nodeName, description, equipmentContext, failureConfig);
