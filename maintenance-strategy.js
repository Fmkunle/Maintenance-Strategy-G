...
const createNode = (type, code = "", name = "", description = "", equipmentContext = null, failureConfig = null, cmConfig = null) => ({
  id: createId(type),
  type,
  code,
  name,
...
const normalizeHierarchyNode = (node, fallbackType = "subsystem", parentFullCode = "") => {
  ...
  const displayName =
    type === "effect" || type === "cm"
      ? localCodeSegment || normalizedName || normalizedCode
      : fullInheritedCode;
...
const updateInheritedCodesForSubtree = (node, parentFullCode = "") => {
  ...
  node.name =
    node.type === "effect" || node.type === "cm"
      ? String(node.code || "").trim()
      : joinInheritedCode(parentFullCode, node.code, node.type);
  (node.children || []).forEach((child) => {
...
const openChildCreator = (parentId, childType = "") => {
  ...
  childDraftState = {
    ...
    cmName: nextChildType === "cm" ? getDefaultCmDraftName(parentId) : "",
  };
};
...
const renderChildCreator = (nodeInfo, actions) => {
  ...
  const showCmTaskEditor = selectedChildType === "cm";
  ...
  if (showCmTaskEditor) {
    const cmCode = getNextAutoGeneratedCode(nodeInfo.node, "cm");
    ...
    childCreatorPanel.innerHTML = `
      <section class="asset-child-creator__form cm-task-editor">
          ...
          <div class="cm-task-editor__stack">
            <div class="cm-task-editor__field cm-task-editor__field--full">
              ...
              <input
                ...
                type="text"
                maxlength="10"
                value="${escapeHtml(childDraftState.cmName || cmCode)}"
                placeholder="Enter CM code"
                required
              >
            </div>
          ${
        ...
        <div class="asset-child-creator__actions">
          ...
          ${
            childDraftState.cmStep === "core"
              ? `<button id="nextCmStepButton" class="primary-button" type="button" ${isCmCoreDraftReady(childDraftState) ? "" : "disabled"}>Next</button>`
...
const createChildNode = (parentId, childType, draft) => {
  ...
  const fullCode = childType === "effect" || childType === "cm" ? codeSegment : joinInheritedCode(parentFullCode, codeSegment, childType);
  const description =
    childType === "effect"
  ...
  const nodeName = childType === "effect" || childType === "cm" ? codeSegment : fullCode;
  const nextNode = createNode(childType, codeSegment, nodeName, description, equipmentContext, failureConfig, cmConfig);
...
const syncChildCreatorDraftField = (target) => {
  ...
  if (target.dataset.childTypeOption) {
    ...
    childDraftState = {
      ...
      ...childDraftState,
      childType: nextChildType,
      codeSegment: nextChildType === "effect" ? "" : childDraftState.codeSegment,
      description: "",
      redundancyMode: "None",
      redundancyPercent: "",
      cmStep: "core",
      cmName: nextChildType === "cm" ? getDefaultCmDraftName(childDraftState.parentId) : "",
      cmResources: nextChildType === "cm" ? [] : childDraftState.cmResources,
      cmSparePartsRequired: nextChildType === "cm" ? [] : childDraftState.cmSparePartsRequired,
      cmToolsRequired: nextChildType === "cm" ? [] : childDraftState.cmToolsRequired,
    };
    renderSelectedNodePanel();
    return;
  }
...
childCreatorPanel?.addEventListener("click", (event) => {
  ...
  if (nextCmStepButton && isCmCoreDraftReady(childDraftState)) {
    childDraftState.cmName = normalizeCmCodeInput(childDraftState.cmName) || childDraftState.cmName;
